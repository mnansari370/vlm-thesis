...
and to measure that trade honestly: frozen backbones, locked evaluation samples, identical prompts
and decoding for every method, and compute accounting that charges every executed forward pass.
...
document pages (DocVQA) — deliberately covering both regimes where pruning is easy and regimes
...
CLS attention in the vision encoder; Qwen ranks merged visual embeddings by activation norm. Tokens
are physically removed before the language model, so the savings are real. Static is the strong
baseline every dynamic method must beat: at the 75 % budget it sits within 0.02–2.63 points of dense
on every model–dataset cell while removing roughly a quarter of the FLOPs, and on GQA and VQAv2 even
the 15 % budget loses only about 4–8 points. It collapses only where information is dense and
spatial: Qwen TextVQA falls 81.06 → 53.06 and Qwen DocVQA 94.76 → 29.90 at the tightest budget.
...
the top K, and restores the original spatial order — training-free, with no extra language-model
forward pass for scoring. Because the budget is identical to static per sample, the comparison
...
accounting is honest — an escalated sample pays for both passes — and the comparison target is the
...
output schema, fairness gate, FLOP accounting — lives in `src/common/`; the per-method runner cores
live in `src/dense/`, `src/static/`, `src/dynamic_which/`, and `src/dynamic_count/`.
...
The headline positive result — Qwen2.5-VL-7B on TextVQA, full 5,000 validation questions:
...
3. **Where it works, it works well.** On Qwen2.5-VL × TextVQA, question-conditioned selection gains
   +7.50 to +8.36 points at tight budgets — a validated, reproducible improvement. The win requires
   localized, question-addressable evidence and language-aligned visual features.
...
5. **The oracle headroom is real but not recovered.** A perfect per-sample budget router would beat
   the best fixed budget by +2.09 to +6.25 points while removing 61–83 % of visual tokens, yet the
   real confidence-based controllers do not reliably recover it under honest two-pass accounting.
...
The completed results can be re-verified on CPU in minutes — no model, no GPU:
...
`dynamic_which/`, `dynamic_count/` — documents its rerun commands in `COMMANDS.md`; all launchers
