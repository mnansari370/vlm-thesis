...
- `results/final_scope/` — the saved per-cell outputs, the committed tables, and the fitted
...
2. **Result-basename breakage.** Static, Dynamic-WHICH, and Dynamic-COUNT resolve their dense /
   static / frozen references by **reconstructing file basenames** under `results/final_scope/`.
...
3. **Audit / table-script breakage.** The audits and table generators hardcode
   `results/final_scope/...` and `results/final_scope/tables/...` paths. A rename requires updating
...
`results/final_scope/`) and use the method-facing folders for clarity. The runtime is fully
...
`results/final_scope/`, a single atomic commit updating every import + `python -m` path + hardcoded
