...
Usage:
  python -m src.evaluation.gqa.run_clip_probe --dataset textvqa_noocr --keep_ks 64,96
  python -m src.evaluation.gqa.run_clip_probe --dataset gqa --keep_ks 64 --max_samples 4000
"""
...
from src.pruning.question_conditioned_selection.clip_select import CLIPSpaceLlava
...
def build_dataset(name, max_samples):
    if name.startswith("textvqa"):
        from src.evaluation.textvqa.run_textvqa import TextVQADataset, JSONL, IMAGE_DIR, collate
        ...
        from src.evaluation.gqa.run_dense_testdev import GQATestdevDataset, collate, IMAGE_DIR
...
def main():
    ...
    out_dir = make_output_dir("results/thesis_main/gqa", args.output_name or f"clip_probe_{args.dataset}")
