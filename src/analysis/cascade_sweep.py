...
from src.analysis.flops import (flops_row, N_TEXT_TESTDEV, N_TEXT_TEXTVQA_OCR,
...
def load_gqa():
    k144 = _preds("results/thesis_main/gqa/testdev_static_cls_attn_k144_*")
    k288 = _preds("results/thesis_main/gqa/testdev_static_cls_attn_k288_*")
    spec = _preds("results/thesis_main/gqa/testdev_speculative_tau055_*")  # has confidence, order matches
...
def load_textvqa(tag, ntext):
    k144d = _latest(f"results/thesis_main/gqa/textvqa_cls_attn_k144_{tag}*") or _latest(f"results/thesis_main/gqa/textvqa_cls_attn_k144_2*") if tag == "ocr" else _latest(f"results/thesis_main/gqa/textvqa_cls_attn_k144_noocr*")
    ...
    if tag == "ocr":
        d144 = _latest("results/thesis_main/gqa/textvqa_cls_attn_k144_2*"); d288 = _latest("results/thesis_main/gqa/textvqa_cls_attn_k288_2*")
    else:
        d144 = _latest("results/thesis_main/gqa/textvqa_cls_attn_k144_noocr*"); d288 = _latest("results/thesis_main/gqa/textvqa_cls_attn_k288_noocr*")
...
def load_pope():
    d144 = _latest("results/thesis_main/gqa/pope_cls_attn_k144_conf_*") or _latest("results/thesis_main/gqa/pope_k144_conf_*")
    d288 = _latest("results/thesis_main/gqa/pope_cls_attn_k288_*")
    if not (d144 and d288):
        return None
...
def load_sqa():
    d144 = _latest("results/thesis_main/gqa/sqa_cls_attn_k144_*"); d288 = _latest("results/thesis_main/gqa/sqa_cls_attn_k288_*")
    if not (d144 and d288):
        return None
...
def main():
    ...
    for ld in loaders:
        ...
        if r:
            out[r["label"]] = r
    with open("results/thesis_main/gqa/cascade_sweep.json", "w") as f:
        json.dump(out, f, indent=2)
    print("\n[saved] results/thesis_main/gqa/cascade_sweep.json")
