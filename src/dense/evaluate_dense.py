...
DESIGN — reuse, don't rewrite:
  ...
  * The actual generation is supplied by a `ModelRunner` (see scripts/dense/run_dense.py)
...
Outputs (docs/DENSE_PROTOCOL.md §12), basename is variant-aware so prompt variants do not collide:
  results/runs/{model}/{dataset}/{basename}.jsonl   (per-sample, §6)
  results/runs/{model}/{dataset}/{basename}.json    (aggregate, §7)
...
def default_basename(dataset: str, n: int, *, use_ocr: bool = True,
    ...
    return base
MANIFEST_DIR = "configs/sample_ids"
