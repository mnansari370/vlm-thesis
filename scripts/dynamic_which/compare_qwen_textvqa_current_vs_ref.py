...
FIRST 200 manifest sample IDs (exactly what the ref n=200 pilots use). Per budget:
  * ref     = results/runs/qwen25vl7b/textvqa/dynamic_which_ref_pilot_n200_textsim_ref_p{b}_ocr_on
...
REF-MISSING (run scripts/dynamic_which/run_qwen_textvqa_ref_validation.sh first).

Writes:
  results/tables/qwen_textvqa_current_vs_ref_validation.csv
  results/tables/qwen_textvqa_current_vs_ref_validation.md
...
MANIFEST = os.path.join("configs", "sample_ids", f"{DATASET}.json")
...
def build_md(rows):
    ...
    if not any_ref:
        out += ["> Ref outputs not present yet. Run "
                "`scripts/dynamic_which/run_qwen_textvqa_ref_validation.sh` (GPU 1, qwen_env), then re-run "
