#!/usr/bin/env bash
# 443K static cache pipeline on GPU 1: K=288, K=192, K=64
# Run AFTER cache_static_gpu1.sh (150K) completes.
...
for K in 288 192 64; do
    ...
    if [ -f "${CACHE_DIR}/train/pooled_features.npy" ]; then
        ...
        log "=== Caching static K=${K} 443K train (GPU 1) ==="
    ...
    # ── MLP training (30 epochs, same as dense 443K) ───────────────────────
...
log "=== GPU 1 443K static pipeline COMPLETE ==="
