...
build_gpu0() {
    for K in 64 144 432; do
        ...
        CUDA_VISIBLE_DEVICES=0 conda run -n vlm_env python scripts/data/cache_features.py \
...
build_gpu1() {
    for K in 128 192; do
        ...
        CUDA_VISIBLE_DEVICES=1 conda run -n vlm_env python scripts/data/cache_features.py \
            --model-type static --keep-tokens $K --split train --config "$CONFIG" \
...
conda run -n vlm_env python scripts/data/check_cache_nan.py --cache-root "$CACHE_DIR"
