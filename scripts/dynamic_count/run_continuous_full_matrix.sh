...
#
# Run:   bash scripts/dynamic_count/run_continuous_full_matrix.sh
...
run_job() {
  ...
  env ${extra_env} CUDA_VISIBLE_DEVICES="${gpu}" "${py}" \
      -m scripts.dynamic_count.run_continuous \
...
echo "All DC-C jobs OK. Next: python -m scripts.tables.make_dynamic_count_tables"
