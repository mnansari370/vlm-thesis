...
Scans results/runs/*/*/dynamic_which_*.json (aggregate JSONs only; never .jsonl and never
...
Emits (real newlines):
  results/tables/dynamic_which_coverage_matrix.csv   (one row per found output + missing cells)
  results/tables/dynamic_which_coverage_matrix.md    (detail + per-cell coverage summary)
...
def build_md(rows: list[dict], summ: list[dict]) -> str:
    ...
    out += [
        "",
        "*Read-only audit of `results/runs/*/*/dynamic_which_*.json`. No result files were "
