...
JSONs under results/final_scope/ (never from memory or prior summaries). Read-only over results;
writes only the table files. No GPU.

Outputs (results/final_scope/tables/):
...
def cow_vs_which_curve():
    ...
    pts = []
    srcs = [f"results/final_scope/qwen25vl7b/textvqa/dynamic_which_final_textsim_p{b}_ocr_on.jsonl"
            for b in BUDGETS] + ["results/final_scope/qwen25vl7b/textvqa/dense_final_ocr_on.jsonl"]
    for s in srcs:
    ...
    out = []
    for p in sorted(glob.glob("results/final_scope/qwen25vl7b/textvqa/dynamic_count_dc*_textsim_*.json")):
        if p.endswith(".jsonl"):
            continue
        a = _agg(p)
        ex = a["extra_dynamic_count"]
...
def write_main(rows, warnings, cow_attr):
    md = ["# Final thesis comparison — Dense / Static / Dynamic-WHICH / DC-D / DC-C",
          "",
          "All numbers read directly from the saved aggregates under `results/final_scope/`. "
