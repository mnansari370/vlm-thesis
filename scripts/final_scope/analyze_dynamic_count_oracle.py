...
Inputs (read-only): the accepted dense + 5 static FINAL per-sample JSONLs for each model×dataset:
  results/final_scope/{model}/{dataset}/{dense_final|static_final_{sel}_p{b}}[variant].jsonl
...
Outputs (real newlines):
  results/final_scope/tables/dynamic_count_oracle_summary.csv
  results/final_scope/tables/dynamic_count_oracle_summary.md
Stdlib only.
"""
...
def _root(model: str, dataset: str) -> str:
    return os.path.join("results", "final_scope", model, dataset)
