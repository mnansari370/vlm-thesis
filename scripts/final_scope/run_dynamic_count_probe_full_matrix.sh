...
#
# Run:   bash scripts/final_scope/run_dynamic_count_probe_full_matrix.sh
...
run_job() {
  ...
  env ${extra_env} CUDA_VISIBLE_DEVICES="${gpu}" "${py}" \
      -m scripts.final_scope.run_dynamic_count_probe \
...
echo "All probe jobs OK. Next: python -m scripts.final_scope.make_dynamic_count_controller_calibration"
