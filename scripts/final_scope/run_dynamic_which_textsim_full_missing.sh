...
#   nohup bash scripts/final_scope/run_dynamic_which_textsim_full_missing.sh \
...
run_cell() {
  ...
  for b in "${BUDGETS[@]}"; do
    ...
    env ${extra_env} CUDA_VISIBLE_DEVICES="${gpu}" "${py}" \
        -m scripts.final_scope.run_dynamic_which_eval \
...
echo "Next: python -m scripts.final_scope.audit_dynamic_which_full_final_matrix"
echo "      python -m scripts.final_scope.make_dynamic_which_textsim_full_final_summary"
