...
#
# Missing cells (from scripts/final_scope/audit_dynamic_which_coverage.py):
...
#
# Run:  bash scripts/final_scope/run_dynamic_which_missing_pilots.sh
# -----------------------------------------------------------------------------
set -uo pipefail
...
run() {
  ...
  env ${extra_env} CUDA_VISIBLE_DEVICES="${gpu}" "${py}" \
      -m scripts.final_scope.run_dynamic_which_eval --n "${N}" "$@" 2>&1 | tee "${log}"
...
echo "Next: python -m scripts.final_scope.audit_dynamic_which_coverage  (re-audit coverage)"
