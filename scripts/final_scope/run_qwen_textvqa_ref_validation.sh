...
#   results/final_scope/qwen25vl7b/textvqa/dynamic_which_ref_pilot_n200_textsim_ref_p{b}_ocr_on.json[l]
#
...
#
# Run:  bash scripts/final_scope/run_qwen_textvqa_ref_validation.sh
# -----------------------------------------------------------------------------
set -uo pipefail
...
OUTDIR="results/final_scope/qwen25vl7b/textvqa"
N=200
...
run() {
  ...
  CUDA_VISIBLE_DEVICES=1 "${QWEN_PY}" \
      -m scripts.final_scope.run_dynamic_which_ref_eval --n "${N}" "$@" 2>&1 | tee "${log}"
...
echo "Next: python -m scripts.final_scope.compare_qwen_textvqa_current_vs_ref"
