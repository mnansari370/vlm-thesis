...
cfg = load_config('configs/dense/llava_dense_150k_10k_fullvocab.yaml')
cfg['training']['epochs'] = 30
set_seed(42, False)

os.makedirs('results/dense_v1_30ep', exist_ok=True)
...
for epoch in range(1, 31):
    ...
    if best_acc is None or val_acc > best_acc:
        ...
        save_checkpoint('results/dense_v1_30ep/best_model.pt', answer_head, optimizer, scheduler, epoch,
                        {'train': tr, 'val': {k:v for k,v in vl.items() if k!='predictions'}})
        save_json('results/dense_v1_30ep/best_predictions.json', {'predictions': vl['predictions']})
    save_json('results/dense_v1_30ep/history.json', history)

save_json('results/dense_v1_30ep/metrics.json', {
...
${PYTHON} -u -m VQA_V2.shared.evaluation.generate_and_score \
    ...
    --checkpoint  results/dense_v1_30ep/best_model.pt \
    --model-type  dense \
    --output-path results/dense_v1_30ep/generation_eval_1k.json \
    --max-samples 1000 \
    --skip-classification \
    --log-every 200 \
...
for K in 64 128 144 192 288 432; do
    ...
    mkdir -p "results/static_k${K}_zeroshot"

    ${PYTHON} -u -m VQA_V2.shared.evaluation.generate_and_score \
        --config      "configs/static/llava_static_clsattn_150k_10k_fullvocab_k${K}.yaml" \
        --model-type  static \
        --output-path "results/static_k${K}_zeroshot/generation_eval_1k.json" \
        --max-samples 1000 \
        --skip-classification \
        --log-every 200 \
    ...
    ${PYTHON} -c "
import json
with open('results/static_k${K}_zeroshot/generation_eval_1k.json') as f:
    d = json.load(f)
...
import json, os
p = 'results/static_k${K}_zeroshot/generation_eval_1k.json'
if os.path.exists(p):
    d = json.load(open(p))
