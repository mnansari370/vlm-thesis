...
$PY -m v2.distill.eval_control --student results/thesis_main/highres/distill/student_12k.pt \
    --gate-split validation --gate-start 0 --n 200 --K 128 --log-every 50 \
    --out results/thesis_main/highres/distill/control_12k.json > "$LOG/eval_control_12k.log" 2>&1 || echo "[fu] control failed (non-fatal)"

echo "[fu] $(date) ===== STAGE 2: retrain 25 epochs (reuse feature cache) ====="
$PY -m v2.distill.train_student --teacher "$TCACHE" --out results/thesis_main/highres/distill/student_12k_25ep.pt \
...
$PY -m v2.distill.eval_gate --student results/thesis_main/highres/distill/student_12k_25ep.pt \
    --gate-split validation --gate-start 0 --n 400 --K 128 --log-every 100 \
    --out results/thesis_main/highres/distill/gate_12k_25ep.json > "$LOG/eval_gate_12k_25ep.log" 2>&1 || { echo "[fu] gate FAILED"; exit 1; }

echo "[fu] $(date) ===== ALL DONE ====="
echo "--- control ---"; cat results/thesis_main/highres/distill/control_12k.json 2>/dev/null
echo "--- gate 25ep ---"; cat results/thesis_main/highres/distill/gate_12k_25ep.json 2>/dev/null
