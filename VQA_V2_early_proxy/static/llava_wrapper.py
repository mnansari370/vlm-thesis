"""Static-pruning wrapper: CLS-attention top-K (fixed K), frozen backbone, trainable answer head."""
...
class LlavaStaticVQAModel(nn.Module):
    """
    Static-pruning LLaVA-style VQA model.

    Current supported mode:
    - classification + answer head

    This rollback version keeps the original static-pruning path that produced
    meaningful results, while preserving eager attention for reliable attention
    extraction.
    """
    ...
    def _run_vision_encoder(
        ...
        final_attentions = attentions[-1]               # [B, H, 1+N, 1+N]
    ...
    def _build_pruned_multimodal_inputs(
        ...
        kept_tokens = projected_image_features.size(1)
        ...
        lm_inputs_embeds = torch.cat(
            [projected_image_features, text_embeds],
            dim=1,
        )
        lm_attention_mask = torch.cat(
            [image_attention_mask, text_attention_mask],
            dim=1,
        )
