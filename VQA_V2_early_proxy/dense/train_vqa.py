"""Training entry point for the dense baseline (60k/top-3500 era). Run: python -m VQA_V2_early_proxy.dense.train_vqa --config <yaml>."""
...
from VQA_V2_early_proxy.dense import LlavaDenseVQAModel
from VQA_V2_early_proxy.dense import (
    build_optimizer,
    build_scheduler,
    measure_latency,
    train_one_epoch,
...
def build_loader(dataset, cfg: Dict[str, Any], split_name: str):
    ...
    Design choice:
    - training split uses shuffle=True
    - validation split uses shuffle=False
    - images remain as PIL images until they are processed inside the model wrapper
    """
...
def save_checkpoint(
    ...
    Since the backbone is frozen in the dense baseline, we mainly care about
    the answer head state and optimizer/scheduler state for reproducibility.
    """
    checkpoint = {
        "epoch": epoch,
        "metrics": metrics,
...
def load_answer_head_checkpoint_if_requestedd(model, cfg: Dict[str, Any]) -> None:
    """
    Load a saved answer-head checkpoint for eval-only runs.

    This is needed when we want to evaluate a previously trained dense baseline
...
def estimate_analytical_attention_flops(
    model: LlavaDenseVQAModel,
    metrics: Dict[str, Any],
) -> Optional[Dict[str, Any]]:
    """
    Analytical attention-only FLOPs proxy.

    This is still the main FLOPs metric for the thesis because it directly
    reflects the multimodal sequence-length cost that the thesis studies.
...
class DenseClassificationFlopsWrapper(nn.Module):
    """
    A small tensor-only wrapper around the dense baseline for fvcore FLOPs.

    Why this wrapper exists:
    - fvcore traces tensor computations
    - the outer dataset/model path includes Python objects like PIL images
      and text strings, which are not trace-friendly
    - this wrapper traces the actual classification-mode forward used by the
      thesis experiment:
          LLaVA backbone -> final hidden state pooling -> answer head
    """

    def __init__(self, dense_model: LlavaDenseVQAModel):
        super().__init__()
    ...
    def forward(
        ...
        outputs = self.dense_model.model(
            input_ids=input_ids,
        ...
        if outputs.hidden_states is None:
            raise ValueError("Backbone did not return hidden states during fvcore FLOPs tracing.")

        last_hidden = outputs.hidden_states[-1]
        pooled_features = self.dense_model._gather_last_valid_hidden(
            hidden_states=last_hidden,
            attention_mask=attention_mask,
        )

        if self.dense_model.answer_head is None:
            return pooled_features

        logits = self.dense_model.answer_head(pooled_features)
        return logits


def _extract_single_sample_batch(batch: Dict[str, Any]) -> Dict[str, Any]:
    """
    Reduce an existing collated batch to a single-sample batch.

    This keeps tracing smaller and more stable for fvcore.
    """
    return {
...
def estimate_fvcore_flops(
    ...
    Notes:
    - This is a best-effort FLOPs estimate.
    - fvcore traces actual tensor ops and may report unsupported operators on
      modern Transformer / Hugging Face models.
    - We save those warnings in the metrics JSON so the result is transparent.

    We treat this as a secondary comparison metric.
    The thesis should still rely primarily on the analytical attention FLOPs.
    """
    try:
    ...
    flops_model = DenseClassificationFlopsWrapper(model)
    flops_model.eval()
    ...
    try:
        ...
        return {
            "available": True,
            "method": "fvcore_traced_dense_classification_forward",
...
def run_eval_only(cfg: Dict[str, Any], output_dir: str):
    """
    Run evaluation-only mode, typically used for debug sanity checks.
    """
    ...
    load_answer_head_checkpoint_if_requestedd(model, cfg)
...
def run_train_answer_head(cfg: Dict[str, Any], output_dir: str):
    """
    Main dense-baseline training path.

    This follows the successful idea from the previous best run:
    - full visual tokens
    - frozen LLaVA backbone
    - train only the answer head
    - classification-mode VQA
    """
