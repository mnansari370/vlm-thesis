"""Training entry point for the dynamic model (60k/top-3500 era, exp1-5). Run: python -m VQA_V2_early_proxy.dynamic.train_vqa --config <yaml>."""
...
from VQA_V2_early_proxy.dynamic import LlavaDynamicVQAModel
from VQA_V2_early_proxy.dynamic import (
    build_optimizer,
    build_scheduler,
    measure_latency,
    train_one_epoch,
...
def save_checkpoint(
    ...
    if getattr(model, "token_selector", None) is not None:
        checkpoint["token_selector_state_dict"] = model.token_selector.state_dict()
...
def estimate_analytical_attention_flops(
    ...
    Uses the actual average multimodal sequence length from validation.
    For dynamic eval, this should come from hard selection.
    """
    ...
    flops = 4.0 * num_layers * (seq_len ** 2) * hidden_size
...
class DynamicClassificationFlopsWrapper(nn.Module):
    """
    Tensor-only wrapper for fvcore FLOPs.

    Traces the dynamic hard-eval path:
        vision tower -> dynamic selector -> projector -> frozen LLM -> answer head

    This is secondary/best-effort, same as dense/static.
    """

    def __init__(self, dynamic_model: LlavaDynamicVQAModel):
        super().__init__()
    ...
    def forward(
        ...
        lm_inputs_embeds, lm_attention_mask, _, _ = (
            self.dynamic_model._build_dynamic_multimodal_inputs(
                model_inputs=model_inputs,
                questions=questions,
            )
        )

        language_model = self.dynamic_model._get_language_model()
        ...
        pooled = self.dynamic_model._gather_last_valid_hidden(
            hidden_states=last_hidden,
            attention_mask=lm_attention_mask,
        )

        logits = self.dynamic_model.answer_head(pooled)
        return logits
...
def estimate_fvcore_flops(
    ...
    flops_model = DynamicClassificationFlopsWrapper(model)
    flops_model.eval()
    ...
    try:
        ...
        return {
            "available": True,
            "method": "fvcore_traced_dynamic_hard_classification_forward",
...
def run_train_dynamic_answer_head(cfg: Dict[str, Any], output_dir: str):
    ...
    print(
        f"[Info] Starting dynamic training for {cfg['training']['epochs']} epochs | "
        f"steps/epoch={len(train_loader)} | "
        f"grad_accum={cfg['training']['grad_accum_steps']} | "
        f"budget_loss_weight={cfg['training'].get('budget_loss_weight', 0.003)}",
        flush=True,
    )
    ...
    for epoch in range(int(cfg["training"]["epochs"])):
        ...
        print(
            ...
            f"train_ce={train_metrics.get('ce_loss')} | "
            f"train_vqa={train_metrics.get('vqa_accuracy')} | "
            f"train_tokens={train_metrics.get('avg_num_visual_tokens_after')} | "
            f"val_loss={val_metrics.get('loss')} | "
            f"val_ce={val_metrics.get('ce_loss')} | "
            f"val_vqa={val_metrics.get('vqa_accuracy')} | "
            f"val_tokens={val_metrics.get('avg_num_visual_tokens_after')}",
            flush=True,
        )
        ...
        elif monitor == "val_ce_loss":
            current_metric = val_metrics.get("ce_loss", None)
        else:
        ...
        if current_metric is not None:
            if best_metric is None:
                is_better = True
            elif monitor in {"val_loss", "val_ce_loss"}:
...
def main():
    ...
    elif training_mode == "train_dynamic_answer_head":
        metrics = run_train_dynamic_answer_head(cfg, output_dir)
    else:
