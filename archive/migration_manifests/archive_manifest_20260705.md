...
- **cleanup pass:** Final polish pass
- **date:** 2026-07-05
- **branch:** method-migration
- **commit before pass:** fef4c9b ("Refactor repository into method-oriented structure")

Small tree tidy-up: removed the last visible leftovers so the root and `src/` show only the
method-oriented structure. **logs/** and the active datasets **data/{gqa,textvqa,vqav2}** were not
touched. No result JSON/JSONL was moved or edited; no result basenames changed.

## `__pycache__` (git-ignored; plain `mv`, not deleted)

All 25 visible `__pycache__` directories (under `src/` and `scripts/`, regenerated by the CPU test
runs of the previous pass) → `archive/local_only/pycache_visible_cleanup_20260705/<original/path>/`,
preserving relative paths. None remain outside `archive/`. Restore: `mv
archive/local_only/pycache_visible_cleanup_20260705/<path> <path>` (or just let Python regenerate them).

## Moved tracked files

| Old path | New path | Reason | Restore |
|---|---|---|---|
| `deep_structure_plan_20260705.md` | `archive/legacy_docs/cleanup_plans/deep_structure_plan_20260705.md` | useful history, but should not sit in the thesis root | `git mv archive/legacy_docs/cleanup_plans/deep_structure_plan_20260705.md deep_structure_plan_20260705.md` |
| `methods_restructure_plan.md` | `archive/legacy_docs/cleanup_plans/methods_restructure_plan.md` | superseded planning doc; history only | `git mv archive/legacy_docs/cleanup_plans/methods_restructure_plan.md methods_restructure_plan.md` |
| `experiments/README.md` (the whole folder) | `archive/legacy_experiments/experiments/README.md` | `experiments/` held only a stale pointer README (no active code); the four method folders + `scripts/` provide that navigation now | `git mv archive/legacy_experiments/experiments experiments` |
| `src/final_scope/` (9 shim modules + `__init__.py` + `README.md`) | `archive/legacy_experiments/src/final_scope_compat/` | the backward-compatibility shim layer; **no active tracked module imports `src.final_scope`** (verified by `git grep`), and the CPU validation suite + both-environment import smokes pass without it, so it was retired from the live tree | `git mv archive/legacy_experiments/src/final_scope_compat src/final_scope` |

## `src/final_scope/` decision

**Archived, not kept.** The shim was verified to have zero active importers: validation now runs
through `python -m src.common.test_evaluation_core` and `python -m scripts.validation.*`, and both
model-environment import smokes use the `src.{dense,static,dynamic_which,dynamic_count}` packages.
Archiving it broke no test. The old `python -m src.final_scope.*` commands no longer resolve; the
old→new import map lives in `final_scope_compat/README.md` and in this manifest's Pass-6 section.

## Verification (after the moves)

`compileall` exit 0 · `src.common.test_evaluation_core` ALL PASSED · `scripts.validation.validate_dynamic_{which,count}`
ALL_*_VALID=True · `scripts.validation.audit_dynamic_which` 40 complete / 0 invalid ·
`scripts.validation.audit_dynamic_count` 0 equivalence failures · both-env import smokes OK.
