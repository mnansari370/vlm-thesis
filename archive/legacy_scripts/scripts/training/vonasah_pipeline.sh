...
for K in 64 128 144 192 288 432; do
    ...
    ${PYTHON} -u scripts/data/cache_features.py \
        --model-type static \
        --keep-tokens "${K}" \
        --split val \
...
${PYTHON} -u -m VQA_V2.shared.evaluation.generate_and_score \
    --config      "configs/dense/llava_dense_150k_10k_fullvocab.yaml" \
    --checkpoint  "results/dense_v1/best_model.pt" \
    --model-type  dense \
    --output-path "results/dense_v1/generation_eval_1k.json" \
    --max-samples 1000 \
    --skip-classification \
    --log-every 100
...
for K in 64 128 144 192 288 432; do
    ...
    ${PYTHON} -u -m VQA_V2.shared.evaluation.generate_and_score \
        --config      "configs/static/llava_static_clsattn_150k_10k_fullvocab_k${K}.yaml" \
        --checkpoint  "results/static_k${K}_v1/best_model.pt" \
        --model-type  static \
        --output-path "results/static_k${K}_v1/generation_eval_1k.json" \
        --max-samples 1000 \
        --skip-classification \
        --log-every 100
