...
All numbers read directly from the saved aggregates under `results/final_scope/`. Dense/static/WHICH rows are FULL-manifest; DC-D/DC-C rows are the held-out 80% eval split (their Δ columns are computed against dense/static over the SAME ids — fair). Pure Dynamic-COUNT (cls_attn/norm) and COUNT-on-WHICH (textsim) are never mixed. Labels vs static curve at matched FLOPs: win > +0.5, near-tie ±0.5, loss < −0.5.
